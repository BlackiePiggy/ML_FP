...
class Tester:
    ...
    def __init__(self, config: Config, checkpoint_path: str):
        ...
        # 新增：与输入一致的“分组版结果”（每个样本dict新增预测字段）
    ...
    def _create_test_loader(self):
        ...
        # 保存原始数据用于可视化和导出（扁平化）
        self.raw_test_data = self._load_raw_test_data_as_flat(self.config.data.test_data_path)

        # 额外：如果 test.pkl 是“分组结构”，保留一份原貌 + 建立扁平索引映射，方便回填预测
        with open(self.config.data.test_data_path, 'rb') as f:
            loaded = pickle.load(f)
        if isinstance(loaded, list) and loaded and isinstance(loaded[0], dict) and "samples" in loaded[0]:
            self.raw_grouped = loaded
            # 为回填预测建立映射（flat_idx -> (group_idx, sample_idx)）
            self.flat_for_grouped, self.index_map = self._flatten_grouped_with_index_map(self.raw_grouped)
            logger.info(f"[Test] GROUPED test set detected: groups={len(self.raw_grouped)}, flattened={len(self.flat_for_grouped)}")
        else:
            self.raw_grouped = None
            self.flat_for_grouped = None
            self.index_map = None
            logger.info("[Test] No grouped structure detected; fallback to flat only.")

        logger.info(f"Test dataset loaded: {len(test_dataset)} samples")

    def _load_raw_test_data_as_flat(self, path):
        with open(path, 'rb') as f:
            data = pickle.load(f)
        if isinstance(data, list) and data and isinstance(data[0], dict) and "samples" in data[0]:
            # 分组格式 → flatten
            flat = []
            for g in data:
                st = g.get("station")
                ...
                for s in g.get("samples", []):
                    if "station_name" not in s and st is not None:
                        s["station_name"] = st
                    if "satellite_prn" not in s and sat is not None:
                        s["satellite_prn"] = sat
                    flat.append(s)
            return flat
        else:
    ...
    def _flatten_grouped_with_index_map(self, grouped):
        """把分组结构 flatten 成样本列表，并建立 flat_idx -> (group_idx, sample_idx) 映射"""
        flat = []
        index_map = []  # 与 flat 同步长度，每个元素是 (group_idx, sample_idx)
        for gi, g in enumerate(grouped):
            samples = g.get("samples", [])
            for si, s in enumerate(samples):
                flat.append(s)
                index_map.append((gi, si))
        return flat, index_map

    def _rebuild_grouped_with_predictions(
            self,
            grouped,
            index_map,
            probs_all,  # np.ndarray or list of [p0, p1]
            pred_logits=None,
            pos_index: int = 1  # 正类下标，默认1
    ):
        import copy, numpy as np
        new_grouped = copy.deepcopy(grouped)
        probs_all = np.asarray(probs_all)

        for flat_idx, (gi, si) in enumerate(index_map):
            s = new_grouped[gi]["samples"][si]

            p = probs_all[flat_idx]  # [p0, p1]
            y_pred = int(np.argmax(p))
            prob_fp = float(p[pos_index])  # 固定“正类=1”的概率（画ROC用）
            prob_pred = float(p[y_pred])  # 预测那一类的概率（直观展示用）

            s["pred_label"] = y_pred
            s["prob_fp"] = prob_fp  # 固定正类概率
            s["prob_predicted"] = prob_pred  # 预测类的概率（与 pred_label 对应）

            if pred_logits is not None:
                s["pred_logit"] = float(pred_logits[flat_idx])

            if "label" in s and s["label"] is not None:
                try:
                    s["is_correct"] = bool(int(s["label"]) == y_pred)
                except Exception:
                    s["is_correct"] = None
            else:
                s["is_correct"] = None
            ...
            # 可选：记录决策规则，方便后续排查
            s["decision_rule"] = "argmax"
            s["pos_index"] = pos_index
    ...
    def test(self):
        ...
        # 可选：检查一个 batch 的 angle_features
        try:
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(self.test_loader, desc="Testing")):
                ...
                # 计算预测和概率（softmax第二列作为“Flex Power=1”的概率）
                ...
                for i in range(batch_size):
                    if data_idx < len(self.raw_test_data):
                        ...
                        data_record = {
                            ...
                            'azimuth': raw_sample.get('azimuth', None),

                            # 名称
                            'station_name': raw_sample.get('station_name', None)
                        }
                        ...
                        metadata = {
                            ...
                            'prob_fp': float(batch_probabilities[i, 1]),         # 固定正类=1 概率
                            'prob_predicted': float(max_prob[i].item()),         # 预测类的概率（与 predicted_label 对应）
                            'confidence': float(max_prob[i].item()),
        ...
        # === 新增：把预测结果“回填”到dx原始的分组结构（与输入一致） ===
        try:
            probs_all = self.results['probabilities']  # shape [N, 2]
            if self.raw_grouped is not None and self.index_map is not None:
                self.grouped_results = self._rebuild_grouped_with_predictions(
                    grouped=self.raw_grouped,
                    index_map=self.index_map,
                    probs_all=probs_all,
                    pred_logits=None,  # 如需
                    pos_index=1  # 若正类列不是1，就改这里
                )
            else:
                # flat 输入 → 也同时存两种概率
                pseudo_group = {
                ...
                for i, s in enumerate(self.raw_test_data):
                    p = probs_all[i]
                    y_pred = int(np.argmax(p))
                    s2 = dict(s)
                    s2["pred_label"] = y_pred
                    s2["prob_fp"] = float(p[1])
                    s2["prob_predicted"] = float(p[y_pred])
                    s2["is_correct"] = (int(s2.get("label", -1)) == y_pred) if (
                                "label" in s and s["label"] is not None) else None
                    s2["decision_rule"] = "argmax"
                    s2["pos_index"] = 1
                    pseudo_group["samples"].append(s2)
                self.grouped_results = [pseudo_group]

        except Exception as e:
            self.grouped_results = None
            logger.exception(f"[WARN] Failed to build grouped_results: {e}")
    ...
    def save_results(self, output_dir: str):
        ...
        # 1b) 保存“分组版”的预测结果（结构与输入一致，每个样本dict新增 pred_* 字段）
        if self.grouped_results is not None:
            grouped_path = os.path.join(output_dir, 'test_results_grouped.pkl')
            with open(grouped_path, 'wb') as f:
                pickle.dump(self.grouped_results, f, protocol=pickle.HIGHEST_PROTOCOL)
            logger.info(f"Grouped-format results saved to {grouped_path}")

            # 可选：逐组单文件保存，便于人工核验
            groups_dir = os.path.join(output_dir, 'groups')
            os.makedirs(groups_dir, exist_ok=True)
            for g in self.grouped_results:
                sta = g.get("station", "unknown")
                sat = g.get("satellite", "unknown")
                day = g.get("date", "unknown")
                fname = f"{sta}_{sat}_{day}.pkl"
                with open(os.path.join(groups_dir, fname), 'wb') as f:
    ...
    def _save_matlab_compatible_data(self, output_dir: str):
        ...
        matlab_data = {
            ...
            'azimuth': [d['azimuth'] if d['azimuth'] is not None else -999 for d in self.results['raw_data']],
            'metadata': self.results['metadata']
        }
    ...
    def _build_timeseries_df(self) -> pd.DataFrame:
        ...
        for r, m in zip(raw, meta):
            ...
            rows.append({
                ...
                'azim': float(azim) if azim is not None else np.nan,
                'true_label': int(m.get('true_label', 0)),
                'pred_label': int(m.get('predicted_label', 0)),
                'prob_on': float(m.get('prob_fp', 0.0)),
...
def main():
    ...
    # 生成“测站-卫星-单日”时间序列图
    tester.plot_all_timeseries(output_dir, draw_s2w_s1c=True)
